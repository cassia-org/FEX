...
static void LoadStateFromWinContext(FEXCore::Core::CPUState& State, void* WowTeb, I386_CONTEXT* Context)
{
  ...
  State.gdt[(Context->SegFs & 0xFFFF) >> 3].base = (uint64_t)WowTeb;
  State.fs_cached = (uint64_t)WowTeb;
  State.es_cached = 0;
  State.cs_cached = 0;
  State.ss_cached = 0;
  State.ds_cached = 0;
...
static void StoreWinContextFromState(FEXCore::Core::CPUState& State, void* WowTeb, I386_CONTEXT* Context)
{
  // General register state
